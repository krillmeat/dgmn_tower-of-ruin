...
class DgmnAttackMenu {
  ...
  clearCursor = (canvas, index) => {
    ...
    canvas.ctx.fillRect( ((4 * 8) + 1) * CFG.screenSize, (2 + (index * 2) ) * tileMod, tileMod - (1 * CFG.screenSize), tileMod );
  }

  paintCursor = (canvas, index) => {
    canvas.paintImage(this.fetchImage('miniCursor'), 4 * ( 8 * CFG.screenSize),  ( 2 + (index * 2) ) * (8 * CFG.screenSize));
  }
  ...
  refreshList = (canvas) => {
    ...
    for(let i = 0; i < loopCount; i++){
      ...
      canvas.ctx.drawImage(this.fetchImage('costLabel'), 5 * (8 * CFG.screenSize), ( 3 + (2 * i) ) * (8 * CFG.screenSize), 16 * CFG.screenSize, 8 * CFG.screenSize);
  
      this.calculateCost(canvas,attack,i);

      canvas.ctx.drawImage(this.fetchImage(`${attack.type}TypeIcon`), 16 * (8 * CFG.screenSize), ( 3 + (2 * i) ) * (8 * CFG.screenSize), 8 * CFG.screenSize, 8 * CFG.screenSize);
      canvas.ctx.drawImage(this.fetchImage(`pwr${attack.power}Icon`), 17 * (8 * CFG.screenSize), ( 3 + (2 * i) ) * (8 * CFG.screenSize), 8 * CFG.screenSize, 8 * CFG.screenSize);
      canvas.ctx.drawImage(this.fetchImage(targetCount), 18 * (8 * CFG.screenSize), ( 3 + (2 * i) ) * (8 * CFG.screenSize), 8 * CFG.screenSize, 8 * CFG.screenSize);
      canvas.ctx.drawImage(this.fetchImage('oneHitIcon'), 19 * (8 * CFG.screenSize), ( 3 + (2 * i) ) * (8 * CFG.screenSize), 8 * CFG.screenSize, 8 * CFG.screenSize);
    }
  }
  ...
  calculateCost = (canvas, attack, index) => {
    ...
    for(let i = 0; i < blockCount; i++){
      ...
      canvas.ctx.drawImage(this.fetchImage(`costMeter${100 + final}`), ( (7 + (i)) * (8 * CFG.screenSize) ), (3 + (index * 2) ) * (8 * CFG.screenSize), (8 * CFG.screenSize), (8 * CFG.screenSize)  );
      remCount -= 4;
    }
  }
}
